...
- POST /api/v1/contacts/import (CSV import with AI field mapping and data scrubbing)
- GET /api/v1/contacts/import/templates (list saved import templates)
- POST /api/v1/contacts/import/templates (save a column mapping as reusable template)
- DELETE /api/v1/contacts/import/templates/:id (delete a saved template)

CSV Import with AI Field Mapping and Saved Templates:

The import flow has 6 steps:

Step 1 - Upload: drag/drop or click to upload CSV. Parse and show row count + detected columns.

Step 2 - AI Field Mapping: analyze column headers AND sample data from first 20 rows to intelligently map to CRM fields. Use fuzzy matching on names ("First Name", "fname", "first", "First_Name", "given name" all map to first_name). Use pattern detection: phone patterns map to phone, @ signs map to email, state abbreviations map to state, ZIP patterns map to zip, dollar amounts map to deal value. Pre-select best match but let user override via dropdown. Low-confidence mappings highlighted in yellow. If a saved template's source columns match the uploaded CSV headers (fuzzy match), auto-suggest that template.

Step 3 - Data Scrubbing: normalize phone numbers to (XXX) XXX-XXXX, lowercase emails, title case names, uppercase state abbreviations, map lead source variations to standard values. Flag rows with missing required fields, invalid emails, invalid phones.

Step 4 - Validation Preview: paginated table with mapped CRM field names as headers. Color code rows: green (clean), yellow (warnings), red (errors). Inline cell editing. Duplicate detection against existing contacts by email or phone. Filter toggles: all, warnings, errors, duplicates.

Step 5 - Import Options: for duplicates choose skip/update/create new. Optional: override lead source for all rows, assign all to a specific rep, auto-create deals. Offer to save this column mapping as a reusable template with a name (e.g., "Home Inspector Leads", "Google Ads Export").

Step 6 - Execute: progress bar, summary (X imported, X updated, X skipped, X failed), download CSV of failed rows, store original file and batch record for undo.

Saved Import Templates: when a user saves a template, store the column mapping, the default lead source, and the source column headers. On future imports, if the CSV headers match a saved template, auto-apply that mapping and skip straight to the validation preview. This turns recurring imports (like weekly home inspector reports) into a three-click process: upload, confirm, done.
