...
export default function AddLead() {
  ...
  const inputClass = 'w-full rounded-lg border border-border bg-white px-3 py-2 text-sm text-heading outline-none focus:border-primary'
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <button
        ...
        className="mb-4 flex items-center gap-1.5 text-sm text-muted-foreground hover:text-heading"
      >
        <ArrowLeft className="h-4 w-4" />
        Back to Accounts
      </button>

      <h1 className="text-lg font-bold text-heading">Add Lead</h1>
      <p className="mt-1 text-sm text-muted-foreground">
        New leads are automatically added to your sales pipeline.
      </p>

      <form onSubmit={handleSubmit} className="mt-6 space-y-6">
        {/* Contact info */}
        <div className="rounded-xl border border-border bg-white p-5 shadow-card">
          <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Contact Info</h2>
        ...
        {/* Property */}
        <div className="rounded-xl border border-border bg-white p-5 shadow-card">
          <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Property</h2>
          <div className="space-y-4">
            <div>
        ...
        {/* Pipeline */}
        <div className="rounded-xl border border-border bg-white p-5 shadow-card">
          <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Pipeline</h2>
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
              ...
              <div>
                ...
                <input
                  ...
                  disabled={skipPipeline}
                  placeholder={`${form.first_name || 'Vaughan'} ${form.last_name || ''} - Home Security`.trim()}
                  className={`${inputClass} ${skipPipeline ? 'opacity-40' : ''}`}
                />
              </div>
            </div>
        ...
        {/* Notes */}
        <div className="rounded-xl border border-border bg-white p-5 shadow-card">
          <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Notes</h2>
          <textarea
            value={form.notes}
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => navigate('/contacts')} className="rounded-lg border border-border px-4 py-2 text-sm text-body hover:bg-page">
            Cancel
          </button>
          <button
            type="submit"
