...
import ActivityTimeline from '@/pages/contacts/components/ActivityTimeline'
import QuickActions from '@/pages/contacts/components/QuickActions'
import BillingConsole from '@/pages/contacts/components/BillingConsole'
...
export default function AccountDetail() {
  ...
  // Tasks
  const contactTasks = useTasksForContact(id ?? '')
  ...
  const contactContracts = useContractsForContact(id ?? '')
  const contactPayments = usePaymentsForContact(id ?? '')
  const contactInvoices = useInvoicesForContact(id ?? '')
  ...
  // Find the contact from store or fetch
  const contact = contacts.find((c) => c.id === id) as Contact | undefined

  useEffect(() => {
    if (!contact && id) fetchContactById(id)
  }, [contact, id, fetchContactById])
  useEffect(() => { if (!contacts.length) fetchContacts() }, [contacts.length, fetchContacts])
  ...
  useEffect(() => { if (id) fetchPayments({ contact_id: id }) }, [id, fetchPayments])
  ...
  const [editForm, setEditForm] = useState({
    ...
    state: '',
    zip: '',
    lead_source: 'other' as Contact['lead_source'],
    notes: '',
  })

  // Sync edit form when contact loads
  useEffect(() => {
    if (contact) {
      setEditForm({
  ...
  if (!contact) {
    return <div className="py-12 text-center text-sm text-muted-foreground">Loading account...</div>
  }
  ...
  async function handleChargeEquipment(amount: number, description: string) {
  ...
  // Billing actions needed calculation
  const billingActions = (() => {
    ...
    const totalEquipmentPaid = contactPayments.filter((p) => p.status === 'succeeded').reduce((sum, p) => sum + p.amount, 0)
    ...
    return { hasEquipment, equipmentOwed, hasMonitoring, monitoringAmount, needsFirstMonthCharge }
  })()
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[340px_1fr]">
        ...
        <div className="space-y-4">
                  ...
                  .map((quote) => (
                    <div key={quote.id} className="rounded-lg border border-border p-2.5">
                      ...
                      <div className="mt-2 flex items-center gap-1.5">
                        {quote.status === 'draft' && (
                          <button
                            onClick={async () => { await sendQuote(quote.id); fetchActivities(contact.id) }}
                        ...
                        {quote.status === 'sent' && (
                          <>
                            <button
                              onClick={async () => { await acceptQuote(quote.id); fetchActivities(contact.id) }}
                              className="rounded bg-success/90 px-2 py-0.5 text-[10px] font-medium text-white hover:bg-success"
                            >
                              Accept
                            </button>
                            <button
                              onClick={async () => { await declineQuote(quote.id); fetchActivities(contact.id) }}
                        ...
                        <button
                          onClick={async () => { await deleteQuote(quote.id) }}
            ...
            {showInlineTask && (
              <div className="mb-3 rounded-lg border border-primary/30 bg-primary/5 p-2 space-y-2">
                <input
                  ...
                  className="w-full bg-transparent text-xs text-heading outline-none placeholder:text-placeholder"
                />
          ...
          {contactInvoices.length > 0 && (
            <div className="rounded-xl border border-border bg-white p-5 shadow-card">
              <h3 className="mb-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground">Invoices</h3>
              <div className="space-y-2">
                {contactInvoices
                  .sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime())
                  .slice(0, 5)
                    ...
                    <div key={payment.id} className="flex items-center justify-between rounded-lg border border-border p-2.5">
                      <div className="min-w-0 flex-1">
                        <p className="text-xs font-medium text-heading">
                          {payment.status === 'succeeded' ? 'Payment approved' : payment.status === 'failed' ? 'Charge failed' : 'Payment'}
                        </p>
                        ...
                        {payment.payment_method_last4 && (
                          <p className="mt-1 truncate text-[10px] text-muted-foreground">
                            Card ending {payment.payment_method_last4}
                          </p>
                        )}
          ...
          {billingActions && (
            <div className="rounded-xl border border-warning/30 bg-warning/5 p-4 shadow-card">
              ...
              <div className="mt-3 space-y-2">
                {billingActions.hasEquipment && (
                  <div className="flex items-center justify-between rounded-lg bg-white border border-border px-3 py-2">
                    <div>
                      <p className="text-xs font-medium text-heading">Equipment & install: {currencyFormat.format(billingActions.equipmentOwed)}</p>
                      <p className="text-[10px] text-muted-foreground">Card on file will be charged</p>
                    </div>
                    <button
                      onClick={() => handleChargeEquipment(billingActions.equipmentOwed, 'Equipment & installation charge')}
                    ...
                    >
                      {billingActionWorking === 'equipment' ? 'Charging...' : `Charge ${currencyFormat.format(billingActions.equipmentOwed)}`}
                    </button>
                  </div>
                )}
                {billingActions.hasMonitoring && (
                  <div className="flex items-center justify-between rounded-lg bg-white border border-border px-3 py-2">
                    <div>
                      <p className="text-xs font-medium text-heading">Monthly monitoring: {currencyFormat.format(billingActions.monitoringAmount)}/mo</p>
                ...
                {billingActions.needsFirstMonthCharge && (
                  <div className="flex items-center justify-between rounded-lg bg-white border border-border px-3 py-2">
                    <div>
                      <p className="text-xs font-medium text-heading">First month monitoring: {currencyFormat.format(billingActions.monitoringAmount)}</p>
                      <p className="text-[10px] text-muted-foreground">Subscription active - charge first month now</p>
                    </div>
                    <button
                      onClick={() => handleChargeEquipment(billingActions.monitoringAmount, 'Monthly monitoring - first month')}
                      disabled={billingActionWorking !== null}
                      className="rounded-md bg-primary px-3 py-1 text-[11px] font-medium text-white hover:bg-primary-hover disabled:opacity-50"
                    >
                      {billingActionWorking === 'monitoring' ? 'Charging...' : `Charge ${currencyFormat.format(billingActions.monitoringAmount)}`}
                    </button>
                  </div>
                )}
              </div>
